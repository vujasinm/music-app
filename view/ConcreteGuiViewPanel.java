...
import cs3500.music.model.RepeatExt;
...
public class ConcreteGuiViewPanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        if (g instanceof Graphics2D) {
            ...
            this.paintRepeats(model, g2);
            this.paintRepeatsExt(model, g2);
        }
    }
    ...
    public void paintRepeats(MusicEditorImpl song, Graphics2D g2) {
        for (Map.Entry<Integer, Repeat> entry : song.getRepeatsDraw().entrySet()) {
            new MyRepeat(entry.getValue(), g2);
        }
    }

    public void paintRepeatsExt(MusicEditorImpl song, Graphics2D g2) {
        for (int i = 0; i < song.getRepeatsExtDraw().size(); i++) {
            new MyRepeatExt(song.getRepeatsExtDraw().get(i), g2);
        }
    }
    ...
    class MyRepeat {
        ...
        public MyRepeat (Repeat rpt, Graphics g2) {
            this.beg = rpt.beg;
            this.end = rpt.end;
            g2.drawLine(80 + beg * 20, 40, 80 + beg * 20, 15);
            g2.drawLine(80 + beg * 20, 15, 80 + end * 20, 15);
            g2.drawLine(80 + end * 20, 40, 80 + end * 20, 15);
        }
    }
    ...
    class MyRepeatExt {
        ...
        public MyRepeatExt (RepeatExt rpt, Graphics g2) {
            new MyRepeat(rpt.commonBody, g2);
            int k = 0;
            for (Map.Entry<Integer, Repeat> entry : rpt.endings.entrySet()) {
                k++;
                g2.drawLine(80 + entry.getValue().beg * 20, 40, 80 + entry.getValue().beg * 20, 15);
                g2.drawLine(80 + entry.getValue().beg * 20, 15, 80 + entry.getValue().end * 20, 15);
                g2.drawLine(80 + entry.getValue().end * 20, 40, 80 + entry.getValue().end * 20, 15);
                g2.drawString(Integer.toString(k), 90 + entry.getValue().beg * 20, 10);
            }
