    ...
    class Task extends TimerTask {
        public void run() {
            if (paused == false) {
                ...
                gui.nextWindowIfEnd();
                curBeat++;
                if (model.getRepeats().containsKey(curBeat - 1)) {
                    int removeKey = curBeat - 1;
                    curBeat = model.getRepeats().get(removeKey).beg;
                    model.getRepeats().remove(removeKey);
                }
                if (model.isCommonBodyEnd(curBeat)) {
                    curBeat = model.getNextEnd(curBeat);
                }
                if (model.isEndingEnd(curBeat)) {
                    curBeat = model.getCommonBodyBeg(curBeat);
                }
            }
            else {
                pause();
