...
public class MusicEditorImpl implements MusicEditorModel {
    ...
    private HashMap<Integer, Repeat> repeats = new HashMap<>();
    public ArrayList<RepeatExt> repeatsExt = new ArrayList<>();
    public ArrayList<RepeatExt> repeatsExtDraw = new ArrayList<>();
    private HashMap<Integer, Repeat> repeatsDraw = new HashMap<>();
    /**
    ...
    @Override
    public void addRepeat(int beg, int end) {
        Repeat rpt = new Repeat(beg, end);
        if (this.checkIfRtOverlaps(rpt.beg, rpt.end) == false) {
            repeats.put(rpt.end, rpt);
            repeatsDraw.put(rpt.end, rpt);
        } else {
            JFrame parent = new JFrame();
            JOptionPane.showMessageDialog(parent, "Repeat overlaps with existing");
        }
    }

    @Override
    public void addRepeatExt(RepeatExt repeat) {
        if (this.checkIfRtOverlaps(repeat.getLowerBound(), repeat.getUpperBound()) == false) {
            repeatsExt.add(repeat);
            repeatsExtDraw.add(repeat);
        }
        else {
            JFrame parent = new JFrame();
            JOptionPane.showMessageDialog(parent, "Repeat overlaps with existing");
    ...
    public static final class Builder implements CompositionBuilder<MusicEditorModel> {
        ...
        @Override
        public CompositionBuilder<MusicEditorModel> addRepeat(int beg, int end) {
            song.addRepeat(beg, end);
            return this;
        }

        @Override
        public CompositionBuilder<MusicEditorModel> addRepeatExt(RepeatExt repeat) {
            song.addRepeatExt(repeat);
            return this;
        }
    }
    ...
    @Override
    public HashMap<Integer, Repeat> getRepeatsDraw() {
        return repeatsDraw;
    }

    @Override
    public ArrayList<RepeatExt> getRepeatsExt() {
        return repeatsExt;
    }

    @Override
    public ArrayList<RepeatExt> getRepeatsExtDraw() {
        return repeatsExtDraw;
    }

    @Override
    public boolean checkIfRtOverlaps(int beg, int end) {
        boolean check = false;
        for (Map.Entry<Integer, Repeat> entry : repeats.entrySet()) {
            for (int i = entry.getValue().beg; i <= entry.getValue().end; i++) {
        ...
        for (int i = 0; i < repeatsExt.size(); i++) {
            for (int j = repeatsExt.get(i).getLowerBound(); j < repeatsExt.get(i).getUpperBound(); j++) {
    ...
    public boolean isCommonBodyEnd(int curbeat) {
        boolean state = false;
        for (int i = 0; i < repeatsExt.size(); i++) {
            if (!repeatsExt.get(i).endings.isEmpty() && curbeat == repeatsExt.get(i).commonBody.end) {
                state = true;
            }
        }
        return state;
    ...
    public int getNextEnd(int curBeat) {
        for (int i = 0; i < repeatsExt.size(); i++) {
            if (!repeatsExt.get(i).endings.isEmpty() && curBeat == repeatsExt.get(i).commonBody.end) {
                int min = Collections.min(repeatsExt.get(i).endings.keySet());
                curBeat =  repeatsExt.get(i).endings.get(min).beg;
            }
        }
        return curBeat;
    }

    public boolean isEndingEnd(int curBeat) {
        boolean state = false;
        for (int i = 0; i < repeatsExt.size(); i++) {
            if (repeatsExt.get(i).endings.containsKey(curBeat)) {
                state = true;
            }
        }
        return state;
    ...
    public int getCommonBodyBeg(int curBeat) {
        for (int i = 0; i < repeatsExt.size(); i++) {
            if (repeatsExt.get(i).endings.containsKey(curBeat) && repeatsExt.get(i).endings.size() > 1) {
                repeatsExt.get(i).endings.remove(curBeat);
                curBeat = repeatsExt.get(i).commonBody.beg;
            }
        }
        return curBeat;
    }
